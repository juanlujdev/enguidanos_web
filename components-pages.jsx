/* global React, Icon, PATRIMONIO, PATRIMONIO_CATS, HISTORIA, HISTORIA_META, EVENTOS, AGENDA_CATS, parseEvDate, expandEventos, TRAMITES, AREAS_CONTENT, ALCALDE, POIS, POI_CAT */
...
function HistoriaLongRead() {
  const [activeId, setActiveId] = useStateP(HISTORIA[0].id);
  const refs = useRefP({});
...
function PatrimonioPhoto({ item, className, children }) {
  const [ok, setOk] = useStateP(false);
...
function PatrimonioPage({ navTarget }) {
  const [cat, setCat] = useStateP("all");
  const [active, setActive] = useStateP(null); // elemento abierto en el detalle
...
function AgendaPage() {
  ...
  const [month, setMonth] = useStateP(new Date()); // arranca en el mes actual
  const [openEv, setOpenEv] = useStateP(null); // índice (dentro de `displayList`) del evento abierto
  const [tab, setTab] = useStateP("prox");      // "prox" = próximos · "pas" = pasados
  const [visible, setVisible] = useStateP(6);    // cuántos se muestran (botón "Ver más")
  useEffectP(() => { setVisible(6); setOpenEv(null); }, [filter, tab]); // reinicia al cambiar filtro o pestaña
...
function AyuntamientoPage({ navTarget }) {
  const [area, setArea] = useStateP(null);
  useEffectP(() => {
...
Object.assign(window, { NaturalezaPage, PatrimonioPage, HistoriaPage, AgendaPage, AyuntamientoPage, OficinaTurismoPage, TurismoPage, TurismoDeportivoPage });
